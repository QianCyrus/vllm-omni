...
def time_metrics_ms(metrics):
    """Keep explicit ms fields; convert seconds and legacy stage_durations."""
    normalized = {}
    ...
    def visit(value, prefix=""):
        ...
        for key, child in value.items():
            ...
            if key == "stage_durations" and isinstance(child, dict):
                for stage, seconds in child.items():
                    if isinstance(seconds, (int, float)) and not isinstance(seconds, bool):
                        normalized[f"{path}.{stage}_ms"] = seconds * 1000.0
            elif isinstance(child, dict):
                visit(child, path)
            elif isinstance(child, (int, float)) and not isinstance(child, bool):
                if key.endswith("_ms"):
                    normalized[path] = float(child)
                elif key.endswith("_s"):
                    normalized[path[:-2] + "_ms"] = float(child) * 1000.0
