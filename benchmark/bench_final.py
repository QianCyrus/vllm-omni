...
from graph_utils import measure_graph
...
def make_strategies(ns):
    ...
    return {"baseline": Baseline(Group()), "coalesced": candidate(Group())}
    ...
    with torch.inference_mode():
        for index, case in enumerate(cases):
            ...
            if args.graphs:
                ...
                for name in order:
                    row["graphs"][name] = measure_graph(
                        lambda: full(name), (q, k, v), measure, args.iterations, args.repeats, args.warmup
                    )
            rows.append(row)
            if rank == 0:
